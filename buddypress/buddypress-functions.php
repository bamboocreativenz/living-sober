...
function bp_legacy_theme_ajax_querystring( $query_string, $object ) {
	...
	$query_string .= empty( $qs ) ? 'per_page=80' : '&per_page=80' ;
	...
	if ( bp_is_user() ): ?>
		<nav class="pagination-sticky members-navigation" role="navigation">
			...
			if ( $prev !== "#" ) {
				...
				<a rel="prev" href="<?php echo bp_core_get_user_domain( $prev ); ?>" title="<?php echo esc_attr( $name ); ?>">
					<span id="older-nav">
			...
			if ( $next !== "#" ) {
				...
				<a rel="next" href="<?php echo bp_core_get_user_domain( $next ); ?>" title="<?php echo esc_attr( $name ); ?>">
				<span id="newer-nav">
					<span class="nav-image"><?php echo bp_core_fetch_avatar( array(
							'item_id' => $next,
							'type'    => 'full'
